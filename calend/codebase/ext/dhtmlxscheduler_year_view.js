...
scheduler.config.year_x = 4, scheduler.config.year_y = 3, scheduler.xy.year_top = 0, scheduler.templates.year_date = function(e) {
    ...
    function() {
        ...
        scheduler.dblclick_dhx_month_head = function(t) {
            if (e()) {
                var a = t.target || t.srcElement,
                    r = scheduler._getClassName(a.parentNode);
                if (-1 != r.indexOf("dhx_before") || -1 != r.indexOf("dhx_after")) return !1;
                for (var n = a; n && (!n.hasAttribute || !n.hasAttribute("date"));) n = n.parentNode;
                if (n) {
                    var i = this.templates.xml_date(n.getAttribute("date"));
                    i.setDate(parseInt(a.innerHTML, 10));
                    var s = this.date.add(i, 1, "day");
                    !this.config.readonly && this.config.dblclick_create && this.addEventNow(i.valueOf(), s.valueOf(), t)
                }
            }
        };
        ...
        var i = scheduler.clear_view;
        scheduler.clear_view = function() {
            if (!e()) return i.apply(this, arguments);
            var t = scheduler._year_marked_cells,
                a = null;
            for (var r in t) t.hasOwnProperty(r) && (a = t[r], a.className = "dhx_month_head",
                a.setAttribute("date", ""));
            ...
            for (var i = this.getEvents(e, this.date.add(e, 1, "day")), s = "", d = 0; d < i.length; d++) {
                var l = i[d];
                if (this.filter_event(l.id, l)) {
                    var o = l.color ? "background:" + l.color + ";" : "",
                        _ = l.textColor ? "color:" + l.textColor + ";" : "";
                    s += "<div class='dhx_tooltip_line' style='" + o + _ + "' event_id='" + i[d].id + "'>", s += "<div class='dhx_tooltip_date' style='" + o + _ + "'>" + (i[d]._timed ? this.templates.event_date(i[d].start_date) : "") + "</div>", s += "<div class='dhx_event_icon icon_details'>&nbsp;</div>", s += this.templates.year_tooltip(i[d].start_date, i[d].end_date, i[d]) + "</div>"
                }
            }
            this._tooltip.style.display = "", this._tooltip.style.top = "0px",
                document.body.offsetWidth - t.left - this._tooltip.offsetWidth < 0 ? this._tooltip.style.left = t.left - this._tooltip.offsetWidth + "px" : this._tooltip.style.left = t.left + r.offsetWidth + "px", this._tooltip.date = e, this._tooltip.innerHTML = s, document.body.offsetHeight - t.top - this._tooltip.offsetHeight < 0 ? this._tooltip.style.top = t.top - this._tooltip.offsetHeight + r.offsetHeight + "px" : this._tooltip.style.top = t.top + "px"
        }, scheduler._year_view_tooltip_handler = function(t) {
            if (e()) {
                var t = t || event,
                    a = t.target || t.srcElement;
                "a" == a.tagName.toLowerCase() && (a = a.parentNode), -1 != scheduler._getClassName(a).indexOf("dhx_year_event") ? scheduler._showToolTip(n(a.getAttribute("date")), scheduler.$domHelpers.getOffset(a), t, a) : scheduler._hideToolTip()
            }
            ...
            return a.querySelector(".dhx_year_body").firstChild.rows[Math.floor(e / 7)].cells[e % 7].firstChild
        }, scheduler._year_marked_cells = {}, scheduler._mark_year_date = function(e, t) {
            var a = r(e),
                n = this._get_year_cell(e),
                i = this.templates.event_class(t.start_date, t.end_date, t);
            scheduler._year_marked_cells[a] || (n.className = "dhx_month_head dhx_year_event",
                n.setAttribute("date", a), scheduler._year_marked_cells[a] = n), n.className += i ? " " + i : ""
            ...
            var t;
            if (e && (t = scheduler.xy.scale_height,
                    scheduler.xy.scale_height = -1), scheduler._els.dhx_cal_header[0].style.display = e ? "none" : "", scheduler.set_sizes(), e && (scheduler.xy.scale_height = t), scheduler._table_view = e, !this._load_mode || !this._load())
                if (e) {
            ...
            for (var o = 7 - (this._ignores_detected || 0), _ = 0, c = 0; 7 > c; c++) this._ignores && this._ignores[c] || (this._cols[c] = Math.floor(i / (o - _)), this._render_x_header(c, s, l, d),
                i -= this._cols[c], s += this._cols[c], _++), l = this.date.add(l, 1, "day");
            d.lastChild.className += " dhx_scale_bar_last";
            for (var c = 0; c < d.childNodes.length; c++) this._waiAria.yearHeadCell(d.childNodes[c]);
            for (var h = this.date[this._mode + "_start"](this.date.copy(this._date)), u = h, v = null, c = 0; c < a.year_y; c++)
                for (var g = 0; g < a.year_x; g++) {
                    v = document.createElement("div"), v.className = "dhx_year_box", v.style.cssText = "position:absolute;", v.setAttribute("date", this.templates.xml_format(h)), v.innerHTML = "<div class='dhx_year_month'></div><div class='dhx_year_grid'><div class='dhx_year_week'>" + d.innerHTML + "</div><div class='dhx_year_body'></div></div>";
                    var f = v.querySelector(".dhx_year_month"),
                        m = v.querySelector(".dhx_year_grid"),
                        p = v.querySelector(".dhx_year_week"),
                        y = v.querySelector(".dhx_year_body"),
                        b = scheduler.uid();
                    this._waiAria.yearHeader(f, b), this._waiAria.yearGrid(m, b), f.innerHTML = this.templates.year_month(h);
                    for (var x = this.date.week_start(h), w = (this._reset_month_scale(y, h, x, 6), y.querySelectorAll("td")), k = 0; k < w.length; k++) this._waiAria.yearDayCell(w[k]);
                    t.appendChild(v), p.style.height = p.childNodes[0].offsetHeight + "px";
                    var D = Math.round((n - 190) / 2);
                    v.style.marginTop = D + "px", this.set_xy(v, r - 10, n - D - 10, r * g + 5, n * c + 5 + scheduler.xy.year_top), e[c * a.year_x + g] = (h.getDay() - (this.config.start_on_monday ? 1 : 0) + 7) % 7, h = this.date.add(h, 1, "month")
                }
        ...
        scheduler.getActionData = function(t) {
            ...
            var a = t ? t.target : event.srcElement,
                ...
                i = scheduler._get_year_day_indexes(n);
            return i && r ? (r = scheduler.date.add(r, i.week, "week"), r = scheduler.date.add(r, i.day, "day")) : r = null, {
                date: r,
                section: null
            }
            ...
            for (var a = 0, r = 0, a = 0, n = t.rows.length; n > a; a++) {
                for (var i = t.rows[a].getElementsByTagName("td"), r = 0, s = i.length; s > r && i[r] != e; r++);
                if (s > r) break
            }
            return n > a ? {
                day: r,
            ...
            return t ? scheduler.date.week_start(scheduler.date.month_start(n(t))) : null
        ...
        scheduler._locate_event = function(e) {
            ...
            if (!t) {
                ...
                var r = n(a.getAttribute("date")),
                    i = scheduler.getEvents(r, scheduler.date.add(r, 1, "day"));
