...
! function() {
    ...
    var r = ["dhx_cal_navline", "dhx_cal_header", "dhx_multi_day", "dhx_cal_data"],
                        ...
                        if (d = n.childNodes[1] && "month" != l ? n.childNodes[1].offsetHeight : Math.max(n.offsetHeight - 1, n.scrollHeight), "month" == l) {
                            ...
                            for (var _, c, u = scheduler.xy.week_agenda_scale_height + scheduler.config.min_grid_size, g = 0; g < n.childNodes.length; g++) {
                                ...
                                for (var h = 0; h < c.childNodes.length; h++) {
                                    for (var f = 0, v = c.childNodes[h].childNodes[1], p = 0; p < v.childNodes.length; p++) f += v.childNodes[p].offsetHeight;
                            ...
                            d = 0;
                            for (var m = n.querySelectorAll(".dhx_map_line"), h = 0; h < m.length; h++) d += m[h].offsetHeight;
                            ...
                            if (d = 0, n.childNodes[1].childNodes[0].childNodes && n.childNodes[1].childNodes[0].childNodes.length) {
                                for (var m = n.childNodes[1].childNodes[0].childNodes[0].childNodes, h = 0; h < m.length; h++) d += m[h].offsetHeight;
        ...
        n = function() {
            ...
            if (s(), scheduler.matrix && scheduler.matrix[e] || "month" == e) {
                var a = window.requestAnimationFrame || window.setTimeout;
                a(function() {
                    s(!0), document.documentElement.scrollTop = i
                }, 1)
            }
        };
    scheduler.attachEvent("onViewChange", n), scheduler.attachEvent("onXLE", n), scheduler.attachEvent("onEventChanged", n), scheduler.attachEvent("onEventCreated", n), scheduler.attachEvent("onEventAdded", n), scheduler.attachEvent("onEventDeleted", n), scheduler.attachEvent("onAfterSchedulerResize", n), scheduler.attachEvent("onClearAll", n),
