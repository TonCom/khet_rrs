...
<html>
    <head>
        ...
        <title>สถานะยกเลิก | จองโดยผู้เช่า</title>
    ...
    <body>
		<?php include "../include/banner.php"; ?>	
			...
			<div class="container-fluid">
				...
				<?php $sql = $db->prepare("SELECT COUNT(ActivityId) AS countacti FROM activity 
                    ...
                    if ($row = $sql->fetch()) { ?> 					
						...
						style="padding: 0 15px 0 15px">ผู้เช่า</span> : สถานะ <span class='label label-danger' style="padding: 0 10px 0 10px">ยกเลิก</span> <?=($row["countacti"]!=0? number_format($row["countacti"]) :  "0" )?> รายการ</b></h3>
				<?php } ?>
			...
			<?php
			$sql = $db->prepare("SELECT ActivityId,CustomerId,RoomId,StartDate,EndDate,ActivityName,Details,ActivityTypeId,IsBreak,FeeBreak,IsLunch,FeeLunch,FeeRoom,FeeOthers,AdminId,LabId,MaidId,RentStatus,CreatedAt,UpdatedAt,ReserveBy FROM activity WHERE RentStatus='cancel' AND ReserveBy='cust' ORDER BY ActivityId DESC");
			$sql->execute();
			$sql->setFetchMode(PDO::FETCH_ASSOC);
			while ($row = $sql->fetch()) {
			echo "<tr>";
				    ...
				    <div class="modal-body">
				        ...
				        <?php if ($row["UpdatedAt"]!='0000-00-00 00:00:00') { ?>
				         <label>แก้ไขล่าสุดเมื่อวันที่</label>&nbsp;&nbsp;<?php echo DateThai($row["UpdatedAt"]) ?> น.<br> <?php } ?>
				        		...
				        		<a href='formeditactivity.php?Id=<?=$row['ActivityId']?>&ctrl=cancel' class="btn btn-primary">แก้ไขข้อมูล</a>			     			        	
				          <button type="button" class="btn btn-danger" data-dismiss="modal">ปิด</button>
				        </div>
				      </div>      
				    </div>
				...
				echo "<td style='padding: 5px'>";
            	   echo "<a href='formeditactivity.php?Id=".$row['ActivityId']."&ctrl=cancel' data-toggle='tooltip' title='แก้ไข'><i style='font-size:22px' class='fa fa-pencil' aria-hidden='true'></i></a>";               
