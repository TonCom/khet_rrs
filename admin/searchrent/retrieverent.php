...
**/
	...
	function active($date,$sttime2,$entime2,$roomid){		
		echo "<td><a href='../adactivity/formaddactivity.php?Startdate=$date $sttime2&Enddate=$date $entime2&Roomid=$roomid' class='btn btn-default btn-sm' style='display:block;  border: 0px'>&nbsp;</a></td>"; 
	}
  ...
  WHERE (
	...
	<?php $sql1 = $db->prepare("SELECT RoomName from room WHERE RoomId=$roomid");
		...
		if ($row1 = $sql1->fetch()) {
			...
			<?php } else { ?>				
					<br><h4><b>สถานะ:</b>&nbsp;&nbsp;<span class='label label-success'>พร้อมให้บริการ</span>&nbsp;&nbsp;<a href='../adactivity/formaddactivity.php?Startdate=<?=$startdate?>&Enddate=<?=$enddate?>&Roomid=<?=$roomid?>'>ต้องการจอง คลิกที่นี่</a></h4>
			<?php	}
		}
} }
  ...
  <?php } else { 
	...
	OR 
		...
		<?php while($row = $sql->fetch()) {							
					...
					echo "<td><a href='../adactivity/formaddactivity.php?Startdate=".$startdate."&Enddate=".$enddate."&Roomid=".$row['RoomId']."' class='btn btn-success btn-sm' style='padding: 5px 20px 5px 20px'>จอง</a></td>"; 
					echo "</tr>";								 
			} ?>
			</tbody>
		</table>
...
// กรอก วันที่ใช้บริการ , ห้อง --> หา เวลาที่ว่าง *          
			 ...
			 if($count<2){
				while($row = $sql->fetch()) {
				  ...
				  if(isset($sttime1)&&isset($entime1)) {
				  	echo "<br><br><a href='../adactivity/formaddactivity.php?Startdate=$date $sttime1&Enddate=$date $entime1&Roomid=$roomid' style='padding-left: 165px'>ต้องการจอง คลิกที่นี่</a>";
				  }
				}
			} 
			else if($count==2){ 
  ...
  <?php } else { 
			...
			<tbody>
			  <tr bgcolor="#FFFFFF">
			  <td>09.00 - 12.00 น.</td>	
	<?php echo "<td><a href='../adactivity/formaddactivity.php?Startdate=$date 09:00&Enddate=$date 12:00&Roomid=$roomid' class='btn btn-success btn-sm' style='padding: 5px 20px 5px 20px'>จอง</a></td>"; ?>		      
	          </tr>
	          <tr bgcolor="#FFFFFF">
			  <td>13.00 - 16.00 น.</td>	
	<?php echo "<td><a href='../adactivity/formaddactivity.php?Startdate=$date 13:00&Enddate=$date 16:00&Roomid=$roomid' class='btn btn-success btn-sm' style='padding: 5px 20px 5px 20px'>จอง</a></td>"; ?>		      
	          </tr>
			  <tr bgcolor="#FFFFFF">
			  <td>09.00 - 16.00 น. (ทั้งวัน)</td>	
	<?php echo "<td><a href='../adactivity/formaddactivity.php?Startdate=$date 09:00&Enddate=$date 16:00&Roomid=$roomid' class='btn btn-success btn-sm' style='padding: 5px 20px 5px 20px'>จอง</a></td>"; ?>		      
	          </tr>
			</tbody>			
		    </table>
  <?php } 
